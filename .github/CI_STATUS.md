...
3. **hvigorw (构建包装脚本)**
   - 项目不维护额外 `hvigorw.bat` 包装脚本
   - 本地和自托管 CI 直接定位 DevEco Studio 自带的 `tools\hvigor\bin\hvigorw.bat`
   - 调用前在当前 shell 临时补齐 DevEco JBR 或 `JAVA_HOME\bin`
...
1. **持续验证 hvigor/JBR 定位链路**
   - 保持 `DEVECOSTUDIO_HOME`、默认安装目录和 `JAVA_HOME` 三种定位链路可用
   - 构建失败时优先检查 DevEco Studio 与 JBR 环境
