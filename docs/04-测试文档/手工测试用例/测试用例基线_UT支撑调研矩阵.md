...
| FW-029 | 规则新增 | 验证新增用户后重新打开新增规则弹窗，用户范围列表包含新用户 | UT部分覆盖 | entry/src/test/firewall/account-change-coordinator.test.ets；entry/src/test/firewall/rules-viewmodel.test.ets；entry/src/test/firewall/system-user-provider.test.ets：覆盖账号变化协调、完整账号列表读取和规则页账号列表刷新。Local UT 不覆盖设备 CommonEvent 投递和弹窗真实打开链路。 |
| FW-030 | 规则新增 | 验证删除用户后重新打开新增规则弹窗，用户范围列表不再包含已删除用户 | UT部分覆盖 | entry/src/test/firewall/account-change-coordinator.test.ets；entry/src/test/firewall/rules-viewmodel.test.ets；entry/src/test/firewall/system-user-provider.test.ets：覆盖账号变化协调、完整账号列表读取和规则页账号列表刷新。Local UT 不覆盖设备 CommonEvent 投递和弹窗真实打开链路。 |
| FW-031 | 用户级策略下发 | 验证新增用户后重新打开全局策略弹窗，目标用户列表包含新用户 | UT部分覆盖 | entry/src/test/firewall/account-change-coordinator.test.ets；entry/src/test/firewall/user-dispatch-viewmodel.test.ets；entry/src/test/firewall/system-user-provider.test.ets：覆盖账号变化协调、完整账号列表读取和用户策略弹窗 ViewModel 消费外部账号列表。Local UT 不覆盖设备 CommonEvent 投递和弹窗真实打开链路。 |
| FW-032 | 用户级策略下发 | 验证删除用户后重新打开全局策略弹窗，目标用户列表不再包含已删除用户 | UT部分覆盖 | entry/src/test/firewall/account-change-coordinator.test.ets；entry/src/test/firewall/user-dispatch-viewmodel.test.ets；entry/src/test/firewall/system-user-provider.test.ets：覆盖账号变化协调、完整账号列表读取和用户策略弹窗 ViewModel 消费外部账号列表。Local UT 不覆盖设备 CommonEvent 投递和弹窗真实打开链路。 |
