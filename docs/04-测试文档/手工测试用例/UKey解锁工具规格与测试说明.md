...
- USB Hub、HID Boot 键盘/鼠标，以及名称明确为 keyboard、mouse、touchpad 的普通输入设备不计入 UKey 候选。
- 无候选时添加失败；存在多把真实候选 UKey 时添加失败；普通键盘鼠标与一把 UKey 同时连接时仍按单把 UKey 处理。
...
| UKEY-DEV-002 | 普通输入外设过滤 | 同时连接 A、键盘、鼠标、Hub | 仍按单把 UKey 处理，普通外设不展示为 UKey |
| UKEY-DEV-003 | 多把真实 UKey | 同时插 A、B 后添加 | 提示只保留一把，不添加凭据 |
