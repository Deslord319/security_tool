...
- **当前约束**: 本轮开放应用管控设置能力：禁止安装走账号维度输入框，只允许用户输入完整应用包名并写入禁止安装列表，不再做应用名或 `appIdentifier` 模糊映射，也不再在当前已安装应用策略行里切换安装策略；目标应用默认不自动选择，显示“请选择应用”，用户选择具体应用后才读取该应用的策略状态和 6D 权限详情，目标应用行左侧显示局部读取转圈，读取完成后刷新“应用策略”和“当前策略”。权限管理应用清单只请求应用基础信息和签名信息，不请求应用图标。卸载保护使用与运行/网络策略一致的下拉框样式，选择不同值后弹确认框再下发；运行策略、网络禁止/恢复和 6D 权限托管由控件选择后直接下发。切换目标应用时必须先同步更新 `selectedAppKey` 并立即刷新“应用策略”区域，6D 权限详情和托管状态读取只能后台补齐，不能阻塞下拉框选中态、策略行状态更新或切换动效；应用策略区域必须按目标应用 key 强制重建，避免 ArkUI 复用旧应用的 Select/状态行内部状态。页面必须在“应用策略”卡片下方提供“当前策略”表格，展示从系统策略列表、当前应用状态和本地成功下发记录合并出的有效策略；系统回读仍是最终真相源，本地记录只用于展示、删除映射和补充 6D 等无法全局枚举的策略。每条可管理策略必须提供删除按钮，点击删除先弹确认框，用户确认后才调用对应 MDM API 清除系统策略，成功后重新回读当前账号策略并清理本地记录。目标应用保活和自启动能力暂不在权限管理页面开放，不读取、不展示、不下发、不在当前策略表生成条目。所有策略写入必须等待 MDM API 返回，写入期间必须按动作粒度设置 `processingActionKey`，仅当前行、当前按钮或当前策略删除项展示 loading；失败时不得提交新 UI 状态，必须保留或恢复原状态并展示失败原因；成功后普通策略只更新当前选中应用的策略状态、摘要和当前策略表格，不刷新应用管控对象区；6D 写入后只回读当前应用的 6D 托管状态。成功不弹二次结果框，失败时才展示失败提示并记录底层错误码。运行策略必须按 `appIdentifier` 下发；卸载保护必须同时覆盖 `bundleName` 和 `appIdentifier`，写入后按系统回读列表确认状态，避免只写单一 ID 导致页面显示与系统卸载入口不一致。安装/卸载策略清理互斥列表时必须先读取现有策略并只移除已存在项，避免参数校验或不存在项清理导致整次失败。当前 SDK 中安装、卸载、运行、网络等企业管理策略读取接口没有 Promise/callback 异步重载，部分 API 直接以 `*Sync` 暴露；因此权限管理 ViewModel 由 `ApplicationRuntimeManager` 统一持有，允许在应用第一次启动且首页入场动画完整结束后随全局 runtime warmup 只预加载账号和应用清单；不得在预热阶段自动选择应用、读取全量策略快照或读取 6D 详情，也不得用固定延迟伪装异步刷新；同一时刻只允许一个当前应用策略快照任务运行。同步新增 `ohos.permission.GET_BUNDLE_INFO` 和 `ohos.permission.ENTERPRISE_MANAGE_USER_GRANT_PERMISSION` 权限及签名模板。直接安装/直接卸载 HAP 文件不作为本模块当前目标，避免把管控工具扩大为应用分发工具。
...
| `switchAccount(accountId)` | 切换账号并重新加载应用、安装/卸载/运行和网络策略状态 |
| `refreshApplications()` | 按当前账号刷新非系统预装应用清单；读取失败时返回权限或管理员激活态失败说明 |
| `selectApplication(appKey)` | 只同步更新选中应用并清空旧策略表和 6D 详情；应用策略区域立即显示当前应用局部读取态 |
| `selectApplicationAndLoadDetails(appKey)` | 先调用 `selectApplication`，再以后台 Promise 读取当前应用策略快照和 6D 权限详情；不得使用固定延时伪装异步，旧应用详情返回时必须丢弃 |
...
flowchart TD
  ...
  D --> E["过滤非系统预装应用"]
  E --> G["展示应用清单和摘要"]
  G --> F["后台读取运行、安装、卸载和网络策略"]
  G --> H["选择应用"]
  H --> I["加载声明权限和 6D 托管状态"]
  H --> J["加载应用控制状态"]
  I --> K["执行权限托管或恢复默认"]
  J --> L["执行安装、卸载、运行和网络策略变更"]
  K --> M["回读策略并刷新页面状态"]
  L --> M
```
...
| 本地策略记录 | `PermissionPolicyLocalRepository` 使用 Preferences 保存成功下发记录 | 仅作展示和删除映射；刷新时系统回读优先，本地失效记录标记 stale 或清理 |
...
| 执行控制 | 可设置默认、允许运行和禁止运行；设置前移除互斥列表，设置后回读 |
| 交互 | 策略控件选择后立即写入并等待 MDM API 返回；成功不弹二次结果框，失败时保留原状态并提示原因 |
| 冲突 | 安装、卸载、运行策略互斥列表写入前必须清理对侧列表，避免系统冲突 |
| 验收 | 输入完整包名可直接写入禁止安装；输入应用名、半截包名或非法包名时拒绝下发并提示；详情区可设置并回读运行 `allowed`、`denied`、`default` 和卸载保护状态；运行策略使用 `appIdentifier` 下发；失败保留原状态并说明原因 |
...
- 目标应用默认不自动选中，未选时“应用策略”和“当前策略”区域展示“请选择应用”。选中应用后，下面用类似身份鉴别页面的 `SectionCard` + `SectionSelectRow` 一条一条展示策略项；卸载保护、运行策略、网络策略均使用下拉框样式，卸载保护选择不同值后通过确认弹框下发。
...
- 策略控件选择后立即下发并等待系统调用完成；普通策略成功后只更新当前应用本地状态和摘要，6D 权限成功后只回读当前应用权限详情。成功不弹二次结果框，失败才弹失败提示，避免确认框和结果框叠加造成重复弹窗感。
- 首次进入页面时，账号和应用清单加载完成后必须先展示主体内容；默认目标应用为空，不自动读取策略列表和 6D 权限详情，不得让全页摘要和管控对象区长期显示“正在加载”。用户选择目标应用后，只针对当前应用刷新策略状态和 6D 权限详情，并在目标应用行左侧展示局部读取转圈。
...
| `SectionSelectRow` | 卸载保护、运行策略、网络策略、6D 托管态选择；卸载保护选择不同值后弹确认框再下发 |
| `SectionSelectRow` / `AsyncSelectRow` | 运行策略、6D 托管态选择 |
...
4. 已接入账号选择、真实应用清单读取、非系统预装过滤、应用选择、安装/卸载/运行策略回读和详情展示；读取失败时提示企业管理员激活态或应用清单权限检查。
