...
`AsyncSelectRow` 只负责本次异步选择的 pending、loading 和失败回退，不承载 `appKey`、策略类型或跨渲染业务状态。下发成功后，ViewModel 更新状态表，页面重新读取父级 `selectedIndex`；失败时状态表不变，组件回退旧值。
...
底层 MDM 调用仍按接口要求传入 `accountId` 参数和 `bundleName` 策略 ID，不能把 `accountId:bundleName` 直接传给系统 bundle 列表 API。网络策略系统侧使用最新 `uid` 下发和清理，但 ViewModel 侧的状态归属仍按 `appKey`。`label` 仅展示，`appIdentifier` 不参与权限管理业务身份、策略匹配、策略写入、删除或去重；即使 SDK 结构字段名为 `appIdentifier`，3D 应用实例中的应用身份值也使用 `bundleName`。
...
| `PermissionAppSnapshot` | `appKey`、`accountId`、`bundleName`、`label`、`uid`、`appIndex`、`uninstallProtected`、`runningBlocked`、`networkBlocked`、`threeDGroups`、`threeDPolicies`、`threeDLoadState`、`threeDMessage` | ViewModel 应用策略状态表行；应用策略区域和当前策略表均从该模型派生 |
...
| `PermissionPageState` | `initialized`、`loading`、`errorMessage`、`selectedAccountId`、`selectedAppKey`、`searchKeyword`、`installBlockKeyword`、`inventoryMessage`、`selectedAppRefreshing`、`policyLoading`、`processingAction`、`processingActionKey`、`accounts`、`appSnapshots`、`installPolicies`、`filteredAppKeys`、`policyRecords`、`summary` | ViewModel 页面状态；策略状态只存放在 `appSnapshots` 和 `installPolicies`，其余表格和摘要均为派生结果 |
