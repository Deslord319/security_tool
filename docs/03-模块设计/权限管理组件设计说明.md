...
| `selectApplication(appKey)` | 同步更新 `selectedAppKey` 与 `selectedApplication` 并清空旧 3D 详情；应用策略区域立即显示当前应用局部读取态，当前策略表保持账号维度展示 |
| `selectApplicationAndLoadDetails(appKey)` | 先调用 `selectApplication` 并提交局部 loading，再在本次选择动作内读取当前应用 3D 权限详情和账号级策略快照；账号级同步策略读取必须走 `taskpool` 或等效后台执行路径，旧应用详情或快照返回时必须丢弃 |
...
- 3D 权限托管使用 `ApplicationInstance`，需要 `appIdentifier`、`accountId`、`appIndex`。
...
| `PermissionPageState` | `adminReady`、`accounts`、`selectedAccountId`、`apps`、`filteredApps`、`selectedAppKey`、`selectedApplication`、`searchKeyword`、`filters`、`summary`、`policyRecords`、`loading`、`policyLoading`、`processingAction`、`processingActionKey`、`error`、`inventoryMessage`、`selectedPermissionProfile` | ViewModel 页面状态；`selectedApplication` 是应用策略区域唯一绑定对象，`processingActionKey` 用于行级 loading 和防重复提交 |
...
| 验收 | 输入完整包名可直接写入禁止安装；输入应用名、半截包名或非法包名时拒绝下发并提示；详情区可设置并回读运行允许（`default`/`allowed` 均显示为允许）、禁止和卸载保护状态；运行策略使用 `appIdentifier` 下发；失败保留原状态并说明原因 |
