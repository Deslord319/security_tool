...
| 版本 | 日期 | 作者 | 变更 |
|---|---|---|---|
| 1.5.27 | 2026-07-21 | Codex | 明确权限管理职责拆分目标：页面保留生命周期、组合和反馈，账号/目标应用区与当前策略表分别由无 ViewModel/Service 依赖的专属组件承接；刷新 Promise 与账号事件尾部补刷由无业务状态依赖的 `PermissionRefreshCoordinator` 承接。状态模型、MDM 调用、页面行为和验收口径不变。 |
