...
> 本文档是 `permission-manage` 权限管理模块的设计与实施契约。当前版本已落地模块登记、路由入口、首页快捷入口、账号选择、真实 MDM 应用清单读取、非系统应用过滤/选择、安装/卸载/运行/网络/保活/自启动策略回读与写入、6D 权限声明识别和托管态设置、失败反馈、文案常量、模型和 ViewModel 状态闭环；页面采用“策略摘要置顶、先选账号再选应用、按身份鉴别样式逐条配置、选择即下发”的管控工具布局。
...
- **当前约束**: 本轮开放应用管控设置能力：安装策略、卸载保护、运行策略、网络禁止/恢复、保活、自启动和 6D 权限托管均由控件选择后直接下发；所有策略写入必须等待 MDM API 返回，成功后普通策略只更新当前选中应用的策略状态和摘要，不刷新应用管控对象区；6D 写入后只回读当前应用的 6D 托管状态。成功不弹二次结果框，失败时才展示失败提示并记录底层错误码。同步新增 `ohos.permission.GET_BUNDLE_INFO` 和 `ohos.permission.ENTERPRISE_MANAGE_USER_GRANT_PERMISSION` 权限及签名模板。直接安装/直接卸载 HAP 文件不作为本模块当前目标，避免把管控工具扩大为应用分发工具。
...
| `PermissionPageState` | `adminReady`、`accounts`、`selectedAccountId`、`apps`、`filteredApps`、`selectedAppKey`、`searchKeyword`、`filters`、`summary`、`loading`、`processingAction`、`error`、`inventoryMessage`、`selectedPermissionProfile` | ViewModel 页面状态 |
...
| 保活/自启动 | `ApplicationControlRepository` 调用 `enterprise.applicationManager.getKeepAliveApps`、`getAutoStartApps`、`addKeepAliveApps`、`addAutoStartApps` | 不作为真相源持久化 |
...
| 安装控制 | 可设置默认、允许安装和禁止安装；设置前移除互斥列表，设置后回读 |
...
| 验收 | 详情区可设置并回读 `allowed`、`denied`、`default` 和卸载保护状态；失败保留原状态并说明原因 |
...
| 保活 | 可使用保活应用列表管理目标 `bundleName` |
| 自启动 | 可使用自启动 `Want` 列表管理目标应用入口 Ability；入口 Ability 查不到时不开放伪设置 |
...
| `ApplicationControlRepository` | 封装安装、卸载、运行黑白名单、保活、自启动策略读写 |
...
- 摘要下方依次选择账号和应用；应用选择使用下拉框，不再展示搜索框，也不再用左侧应用列表 + 右侧详情卡片。
...
- 不可下发的控件必须显示原因，例如“目标应用未声明该权限”或“设备不支持保活能力”；6D 未声明/不可用行保持选择框外观一致，避免按钮区域突然消失。
...
4. 已接入账号选择、真实应用清单读取、非系统预装过滤、应用选择、安装/卸载/运行/保活/自启动策略回读和详情展示；读取失败时提示企业管理员激活态或应用清单权限检查。
5. 实现安装策略、卸载保护和运行策略写入，先不暴露直接卸载。
6. 实现 6D 权限声明识别、托管态回读和已声明权限写入。
7. 实现应用网络策略，先做全部网络禁止/恢复，域名规则作为增强。
8. 实现保活和自启动，处理入口 Ability 缺失和运行策略冲突。
