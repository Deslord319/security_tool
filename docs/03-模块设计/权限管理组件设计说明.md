...
- 首次进入页面时，账号和应用清单加载完成后必须先展示主体内容；之后每次重新进入权限管理页面只刷新当前账号应用清单，避免安装、卸载或重装后目标应用下拉仍使用旧缓存，但不联动强制回读已选应用的 3D 详情或账号级策略快照；如果目标应用已被卸载或过滤掉，则清空目标应用和详情。默认目标应用为空，不自动读取策略列表和 3D 权限详情，不得让全页摘要和管控对象区长期显示“正在加载”。用户选择目标应用后，先同步显示选中态和局部读取转圈，再在本次选择动作内读取当前应用 3D 权限详情和账号级策略快照；同步读取前由下拉组件让 loading/置灰状态先渲染，不能改成 fire-and-forget 后台补齐。
