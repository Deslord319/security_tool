...
> 本文档是 `permission-manage` 权限管理模块的设计与实施契约。当前首版仅落地模块登记、路由入口、首页快捷入口、只读骨架页、文案常量、模型和 ViewModel 初始状态；权限声明、签名模板、高风险写操作和真实 MDM 能力仍未开放。
...
- **目标 Service / Repository**: `entry/src/main/ets/services/permission-manage/PermissionService.ets`；后续真实系统能力接入时再拆分 `PermissionRepository.ets` 和按能力拆分的仓储实现。
...
- **当前约束**: 本轮只接入可导航的只读骨架页，不修改权限声明，不调整签名模板，不暴露安装、卸载、运行、6D 权限、网络、保活和自启动写操作。
...
| Service | `entry/src/main/ets/services/permission-manage/` | 首版提供只读初始态；真实应用清单、应用控制、6D 权限、网络策略、保活策略后续分阶段接入 |
...
| 接口 | 说明 |
|---|---|
| `loadInitialState()` | 初始化管理员状态、账号列表、默认账号应用清单和策略摘要 |
| `switchAccount(accountId)` | 切换账号并重新加载应用、运行策略、网络策略和保活状态 |
| `refreshApplications()` | 按当前账号刷新非系统预装应用清单 |
| `selectApplication(appKey)` | 选中应用并加载详情、6D 权限状态和策略状态 |
...
| `PermissionPageState` | `adminReady`、`accounts`、`selectedAccountId`、`apps`、`selectedAppKey`、`filters`、`summary`、`loading`、`processingAction`、`error` | ViewModel 页面状态 |
...
| 数据 | 真相源 | 本地缓存策略 |
|---|---|---|
| 已安装应用清单 | `enterprise.bundleManager.getInstalledBundleList(admin, accountId)` | 不长期缓存，页面刷新重新读取 |
| 应用声明权限 | 系统包管理查询结果 | 不长期缓存，可在一次页面会话内缓存 |
| 安装/卸载策略 | `enterprise.bundleManager` 安装策略 API | 不作为真相源持久化 |
| 运行策略 | `enterprise.applicationManager` 运行黑白名单 API | 不作为真相源持久化 |
...
| 保活/自启动 | `enterprise.applicationManager.getKeepAliveApps`、`getAutoStartApps` | 不作为真相源持久化 |
...
| 处理 | 读取账号内应用清单，过滤 `systemApp === false` 的非系统预装应用；排除 SecurityTool 自身的高风险操作 |
| 输出 | 应用列表、摘要卡、空态或失败态 |
...
| 安装控制 | 读取和设置安装允许列表、禁止列表、应用分发类型策略 |
| 卸载控制 | 支持加入或移出禁止卸载列表；直接卸载作为高风险操作，首期不默认暴露 |
| 执行控制 | 首期优先支持运行黑名单；运行白名单和 Kiosk 类能力作为二期增强 |
| 交互 | 每次写入前弹确认；写入后回读策略；失败时回滚 UI |
| 冲突 | 运行禁止与保活、自启动互斥，页面必须提示并阻止明显冲突组合 |
| 验收 | 运行禁止后目标应用不可启动；移除策略后恢复默认；卸载保护生效且可撤销 |
...
| 保活 | 使用保活应用列表管理目标 `bundleName` |
| 自启动 | 使用自启动 `Want` 列表管理目标应用入口 Ability |
...
| `ApplicationControlRepository` | 封装安装、卸载、运行黑白名单和高风险操作 |
...
3. 首版页面只展示模块范围、账号/应用摘要空态和管理员依赖提示，不执行任何 MDM 写操作。
4. 实现账号选择、应用清单读取、非系统预装过滤和应用详情展示。
...
- 应用清单只展示非系统预装应用，且不允许对 SecurityTool 自身执行高风险操作。
