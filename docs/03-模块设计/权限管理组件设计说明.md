...
> 本文档是 `permission-manage` 权限管理模块的设计与实施契约。当前首版已落地模块登记、路由入口、首页快捷入口、账号选择、真实 MDM 应用清单读取、非系统应用过滤/搜索/选择、安装/卸载/运行/保活/自启动策略只读回读、只读清单页、文案常量、模型和 ViewModel 初始状态；页面不再展示过程性的“首版范围”说明，高风险写操作仍未开放。
...
- 页头下方放账号选择器、摘要、搜索框和过滤器，不展示过程性的“首版范围”说明卡片。
