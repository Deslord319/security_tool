...
为解决目标应用切换卡顿、已下发策略读回仍显示默认、删除/还原后上下状态不同步的问题，本轮设计以 ViewModel 作为唯一业务状态源，`AsyncSelectRow` 不再维护乐观选中值或成功后等待父组件回填值；下拉框业务值来自父级传入的 `selectedIndex`。组件允许持有本次选择事件的本地 pending guard 和旧值镜像，用于避免重复触发、立即显示行内 loading，并在失败或异常时显式回退到操作前的旧 index；组件不得提前显示目标新值，只能在 `onSelectAsync` 返回成功后切到本次选择值。策略行只有在 MDM 成功并由 ViewModel 归并状态区后才更新 `selectedIndex`，失败时保持旧值。

目标应用选择必须先提交 `selectedAppKey`、`selectedAppRefreshing` 和 `policyLoading`，让目标应用行与应用策略区域进入局部 loading/置灰状态，再执行同步 MDM 读取。由于当前 SDK 的安装、卸载、运行、网络和 3D 状态读取接口存在同步调用，允许在同一个选择 Promise 内等待一个 UI 渲染边界后继续执行系统读取；该等待只用于让已提交的 loading 状态落屏，不作为业务延迟或后台补齐。选择链路必须在同一次动作内完成账号级策略快照和当前应用 3D 详情读取，并通过请求序号丢弃旧选择返回的数据。

当前 SDK 中部分 MDM 读写接口为同步接口或会在 UI 线程产生明显等待，页面层必须使用 `TimedLoadingDialogRunner` 包裹目标应用切换、当前账号切换、策略下发、禁止安装和当前策略删除等耗时动作；动作开始后仍按行级 `processingActionKey` 或 `selectedAppRefreshing` 展示局部 loading，并置灰对应下拉框或按钮，弹窗只用于承接无法被行内 loading 及时落屏的同步耗时。
...
| `selectApplication(appKey)` | 只同步更新选中应用并清空旧 3D 详情；应用策略区域立即显示当前应用局部读取态，当前策略表保持账号维度展示 |
| `selectApplicationAndLoadDetails(appKey)` | 先调用 `selectApplication` 并提交局部 loading，再在本次选择动作内依次读取当前应用 3D 权限详情和账号级策略快照；旧应用详情或快照返回时必须丢弃 |
...
| `PermissionPageState` | `adminReady`、`accounts`、`selectedAccountId`、`apps`、`filteredApps`、`selectedAppKey`、`searchKeyword`、`filters`、`summary`、`policyRecords`、`loading`、`policyLoading`、`processingAction`、`processingActionKey`、`error`、`inventoryMessage`、`selectedPermissionProfile` | ViewModel 页面状态；`processingActionKey` 用于行级 loading 和防重复提交 |
