...
> 本文档是 `permission-manage` 权限管理模块的设计与实施契约。当前版本已落地模块登记、路由入口、首页快捷入口、账号选择、真实 MDM 应用清单读取、非系统应用过滤/选择、按输入包名禁止安装、当前策略表格、策略删除、卸载/运行/网络策略回读与写入、6D 权限声明识别和托管态设置、失败反馈、文案常量、模型和 ViewModel 状态闭环；页面采用“策略摘要置顶、先选账号再选应用、按身份鉴别样式逐条配置、选择即下发”的管控工具布局，当前策略表格参考外设管理“设备连接记录”列表样式并提供删除动作，6D 目录行在详情读取中或读取失败时仍保留占位展示，不允许整块消失；未声明或不可用的 6D 目录不允许展开选择。目标应用保活和自启动不在权限管理页面开放。
...
- **当前约束**: 本轮开放应用管控设置能力：禁止安装走账号维度输入框，只允许用户输入完整应用包名并写入禁止安装列表，不再做应用名或 `appIdentifier` 模糊映射，也不再在当前已安装应用策略行里切换安装策略；目标应用默认不自动选择，下拉框第一项固定为“请选择应用”；用户选择具体应用后控件显示目标应用名称，不再显示“请选择应用”占位文本，下拉框宽度与禁止安装输入框一致；每次进入权限管理页面都必须按当前账号重新读取一次应用清单，确保新安装、卸载或重装的应用能进入目标应用下拉列表，但只刷新应用清单，不联动读取当前选中应用的 6D 详情或账号级策略快照；目标应用列表必须过滤 `systemApp === true` 的系统应用和 SecurityTool 自身，FormRenderService、ArkWeb、长时任务通知资源等系统框架或资源包也必须强制隐藏；不得再用 `removable === false` 隐藏目标应用，避免企业包或调试包被误伤；用户选择具体应用后先同步提交选中态并显示局部读取转圈，目标应用行按外设管理下拉框样式在下拉框左侧显示局部读取转圈，读取期间下拉框置灰；切换目标应用必须在本次选择动作内读取当前应用 6D 权限详情和账号级策略快照，确保应用策略行与当前策略表同步更新；读取前先提交局部 loading/置灰状态，避免同步 MDM 接口执行期间无反馈。“当前策略”表格按当前账号展示所有应用和安装策略，不按目标应用过滤。权限管理应用清单只请求应用基础信息和签名信息，不请求应用图标。卸载保护、运行策略、网络策略和 6D 权限托管均使用外设接口管控同款 `AsyncSelectRow`，下拉过程中左侧显示 loading、控件置灰并在失败时回滚；卸载保护默认“未启用”且不再弹二次确认；运行策略和网络策略下拉只展示“允许/禁止”，其中“允许”对应系统默认放行/恢复默认，不额外展示“默认”选项；6D 未声明、不可用或读取中的目录行不得显示“默认”，必须显示“未声明”或“不可用”等真实不可下发状态。切换目标应用时必须先同步更新 `selectedAppKey` 和局部 loading 并立即刷新“应用策略”区域，6D 权限详情、托管状态和账号级策略快照必须在目标应用切换链路内完成读取，不能改成 fire-and-forget 后台补齐；应用策略区域必须按目标应用 key 强制重建，避免 ArkUI 复用旧应用的 Select/状态行内部状态。页面必须在“应用策略”卡片下方提供“当前策略”表格，展示从系统策略列表、当前账号下所有应用状态和本地成功下发记录合并出的有效策略；系统回读仍是最终真相源，安装、卸载、运行和网络等系统可枚举策略不得用本地记录补回，避免恢复默认后陈旧记录残留；本地记录只用于删除映射和补充 6D 等无法全局枚举的策略。每条可管理策略必须提供删除按钮，点击删除先弹确认框，用户确认后才调用对应 MDM API 清除系统策略；删除处理中在按钮文字左侧展示 loading，不替换“删除”文字，成功后重新回读当前账号策略并清理本地记录。6D 权限行副标题必须说明目录/资源授权用途，不使用“未声明”作为副标题；未声明仅通过禁用态和状态值表达。目标应用保活和自启动能力暂不在权限管理页面开放，不读取、不展示、不下发、不在当前策略表生成条目。所有策略写入必须等待 MDM API 返回，本地策略记录只能在 MDM API 返回成功后写入；恢复默认、关闭卸载保护或删除当前策略成功后，必须清理对应策略记录并按系统回读刷新上方应用策略行；如果系统回读仍短暂返回旧状态，页面必须在成功回调内对当前 action 做一次精确本地同步，保证当前策略表和上方应用策略行立即一致。写入期间必须按动作粒度设置 `processingActionKey`，仅当前行、当前按钮或当前策略删除项展示 loading；失败时不得提交新 UI 状态，必须保留或恢复原状态并展示失败原因；成功后普通策略只更新当前选中应用的策略状态、摘要和当前策略表格，不刷新应用管控对象区；6D 写入后只回读当前应用的 6D 托管状态。成功不弹二次结果框，失败时才展示失败提示并记录底层错误码。运行策略下发禁止时必须按 `appIdentifier` 写入，但恢复默认时必须同时清理 `bundleName` 和 `appIdentifier`，兼容历史或系统残留策略；网络策略恢复默认必须清理当前 UID 下所有 DENY/REJECT 规则，不能只删除一个模板规则；卸载保护必须同时覆盖 `bundleName` 和 `appIdentifier`，写入后按系统回读列表确认状态，避免只写单一 ID 导致页面显示与系统卸载入口不一致。安装/卸载策略清理互斥列表时必须先读取现有策略并只移除已存在项，避免参数校验或不存在项清理导致整次失败。当前 SDK 中安装、卸载、运行、网络等企业管理策略读取接口没有 Promise/callback 异步重载，部分 API 直接以 `*Sync` 暴露；因此权限管理 ViewModel 由 `ApplicationRuntimeManager` 统一持有，允许在应用第一次启动且首页入场动画完整结束后随全局 runtime warmup 只预加载账号和应用清单；不得在预热阶段自动选择应用、读取全量策略快照或读取 6D 详情，也不得用固定延迟伪装异步刷新；同步接口读取前必须先提交局部 loading/置灰状态，避免点击下拉框后无反馈。`IconTextActionButton` 不承载 loading 状态，按钮左侧 loading 由业务页面自行组合。同步新增 `ohos.permission.GET_BUNDLE_INFO` 和 `ohos.permission.ENTERPRISE_MANAGE_USER_GRANT_PERMISSION` 权限及签名模板。直接安装/直接卸载 HAP 文件不作为本模块当前目标，避免把管控工具扩大为应用分发工具。
...
4. **6D 权限托管**: 对目标应用已声明的 6D 目录权限进行托管态设置和回读。
...
- 对应用安装、卸载、运行、网络和 6D 权限进行只读展示与可控写入。
...
| Service | `entry/src/main/ets/services/permission-manage/` | 提供账号、真实应用清单、应用控制策略、网络策略和 6D 权限托管读写门面 |
...
| 接口 | 说明 |
|---|---|
| `loadInitialState()` | 初始化管理员状态、账号列表、默认账号应用清单和基础摘要；不自动选择目标应用，不读取策略快照和 6D 详情 |
| `switchAccount(accountId)` | 切换账号并重新加载应用清单，清空当前选中应用和当前策略表；策略快照由后续目标应用选择、写操作或删除操作触发 |
| `refreshApplications()` | 按当前账号刷新非系统应用清单；账号切换等主动刷新使用，不读取当前选中应用 6D 详情和账号策略快照 |
| `refreshApplicationList()` | 页面每次重新进入时调用，只刷新目标应用清单并保留当前策略表，不联动强制刷新选中应用详情 |
| `selectApplication(appKey)` | 只同步更新选中应用并清空旧 6D 详情；应用策略区域立即显示当前应用局部读取态，当前策略表保持账号维度展示 |
| `selectApplicationAndLoadDetails(appKey)` | 先调用 `selectApplication` 并提交局部 loading，再在本次选择动作内依次读取当前应用 6D 权限详情和账号级策略快照；旧应用详情或快照返回时必须丢弃 |
...
| `setSixDPermissionState(appKey, groupKey, state)` | 设置 6D 权限托管状态，目标应用未声明权限时不可下发并返回明确失败 |
...
- 选中应用后通过系统包管理查询 `reqPermissionDetails`，只对目标应用已声明的 6D 权限开放设置。
- 如果目标应用声明 `ohos.permission.READ_WRITE_USER_FILE`，视为具备全盘文件权限；6D 各目录组显示为可托管，并以该全盘权限作为托管权限名。
- 由 SecurityTool 使用 `securityManager.setPermissionManagedState` 设置 `GRANTED`、`DENIED`、`DEFAULT`。
- 写入后必须使用 `getPermissionManagedState` 回读；未声明权限、包详情查询失败或设备能力不支持时展示短状态文案，控件保持一致外观，但下发必须返回明确失败。
- 后续专项 POC 可补一个声明下载或图库权限的测试应用，用于验证托管态对目标应用授权弹窗和访问结果的实际影响。
...
| 6D 托管状态 | `SixDPermissionRepository` 调用 `enterprise.securityManager.getPermissionManagedState` / `setPermissionManagedState` | 不作为真相源持久化 |
...
| 本地策略记录 | `PermissionPolicyLocalRepository` 使用 Preferences 保存成功下发记录 | 仅作删除映射和 6D 展示补充；安装、卸载、运行、网络等系统可枚举策略刷新时以系统回读为准，不从本地记录补回 |
...
| 交互 | 6D 卡片按目录组展示，未声明显示“未声明”，控件保持选择框外观但不可实际下发；已声明显示当前托管态 |
...
- 目标应用默认不自动选中，未选时“应用策略”展示“请选择应用”；一旦选中具体应用，目标应用控件显示目标应用名称，不再显示“请选择应用”占位文本。选中应用后，下面用类似身份鉴别页面的 `SectionCard` 一条一条展示策略项；目标应用、卸载保护、运行策略、网络策略和 6D 托管态均使用外设接口管控同款 `AsyncSelectRow`，卸载保护默认“未启用”且选择后直接下发，不再弹确认框；运行/网络策略只展示“允许/禁止”，允许对应恢复默认放行；6D 未声明/不可用/读取中行不得显示为“默认”。
- 应用策略卡片下方展示“当前策略”表格，采用外设管理“设备连接记录”的表头、行距、圆角和 hover 风格，列为策略类型、对象、状态、作用范围、操作；表格数据来自 MDM 策略回读和当前应用状态派生，不展示本地未确认历史。
- 6D 权限托管合并到“应用策略”卡片内，不单独占一个卡片，也不在 6D 行上方重复展示“未声明 6D 权限”提示；权限详情加载中不额外插入“处理中”文本，超时或失败时展示不可用行。
- 6D 目录行属于固定策略入口：权限详情为 `null`、读取中、读取失败或目标应用未声明权限时，页面仍必须展示下载目录、桌面目录、文档目录、图库、音乐、文档文件 6 行，并通过行内短描述和禁用选择框表达不可设置原因，不得用空白占位隐藏整组目录。
- 未声明、读取失败、读取中或设备能力不可用的 6D 目录行不得允许展开选择；只有 `canManage=true` 的行可以下发托管态。
- 应用策略每行必须能看到当前状态，避免选择后只能从控件状态间接判断。
- 策略控件选择后立即下发并等待系统调用完成；普通策略成功后只更新当前应用本地状态和摘要，6D 权限成功后只回读当前应用权限详情；本地策略记录只在成功后写入，默认/允许恢复默认和关闭卸载保护不生成记录。成功不弹二次结果框，失败才弹失败提示，避免确认框和结果框叠加造成重复弹窗感。
- 首次进入页面时，账号和应用清单加载完成后必须先展示主体内容；之后每次重新进入权限管理页面只刷新当前账号应用清单，避免安装、卸载或重装后目标应用下拉仍使用旧缓存，但不联动强制回读已选应用的 6D 详情或账号级策略快照；如果目标应用已被卸载或过滤掉，则清空目标应用和详情。默认目标应用为空，不自动读取策略列表和 6D 权限详情，不得让全页摘要和管控对象区长期显示“正在加载”。用户选择目标应用后，先同步显示选中态和局部读取转圈，再在本次选择动作内读取当前应用 6D 权限详情和账号级策略快照；同步读取前由下拉组件让 loading/置灰状态先渲染，不能改成 fire-and-forget 后台补齐。
...
| `MetricSummaryCard` / `CompactSummaryCard` | 应用总数、已管控、6D 敏感、网络限制 |
| `LoadingStatePanel` | 初始加载和局部刷新加载态 |
| `EmptyStatePanel` | 无可管控非系统应用、无 6D 权限、无网络策略 |
| `SettingsSectionCard` | 应用详情里的控制分组 |
| `SectionSelectRow` | 当前账号等无需失败回滚的普通选择行 |
| `AsyncSelectRow` | 目标应用、卸载保护、运行策略、网络策略、6D 托管态选择；写入中左侧 loading、置灰并失败回滚 |
...
- 不可下发的控件必须显示原因，例如“目标应用未声明该权限”或“未读取到有效 UID”；6D 未声明/不可用行保持选择框外观一致，避免按钮区域突然消失。
...
| 层级 | 目标路径 | 覆盖 |
|---|---|---|
| UT | `entry/src/test/permission-manage/` | 系统应用和自应用过滤、账号切换、策略合并、6D 映射、冲突判断 |
