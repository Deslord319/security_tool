...
- **当前约束**: 本轮开放应用管控设置能力：禁止安装走账号维度输入框，只允许用户输入完整应用包名并写入禁止安装列表，不再做应用名或 `appIdentifier` 模糊映射，也不再在当前已安装应用策略行里切换安装策略；目标应用默认不自动选择，下拉框第一项固定为“请选择应用”；用户选择具体应用后控件显示目标应用名称，不再显示“请选择应用”占位文本，下拉框宽度与禁止安装输入框一致；每次进入权限管理页面都必须按当前账号重新读取一次应用清单，确保新安装、卸载或重装的应用能进入目标应用下拉列表，但只刷新应用清单，不联动读取当前选中应用的 3D 详情或账号级策略快照；目标应用列表必须过滤 `systemApp === true` 的系统应用和 SecurityTool 自身，FormRenderService、ArkWeb、长时任务通知资源等系统框架或资源包也必须强制隐藏；不得再用 `removable === false` 隐藏目标应用，避免企业包或调试包被误伤；用户选择具体应用后先同步提交选中态并显示局部读取转圈，目标应用行按外设管理下拉框样式在下拉框左侧显示局部读取转圈，读取期间下拉框置灰；切换目标应用必须在本次选择动作内读取当前应用 3D 权限详情和账号级策略快照，确保应用策略行与当前策略表同步更新；读取前先提交局部 loading/置灰状态，避免同步 MDM 接口执行期间无反馈。“当前策略”表格按当前账号展示所有应用和安装策略，不按目标应用过滤。权限管理应用清单只请求应用基础信息和签名信息，不请求应用图标。卸载保护、运行策略、网络策略和 3D 权限托管均使用外设接口管控同款 `AsyncSelectRow`，下拉过程中左侧显示 loading、控件置灰并在失败时回滚；卸载保护默认“未启用”且不再弹二次确认；运行策略和网络策略下拉只展示“允许/禁止”，其中“允许”对应系统默认放行/恢复默认，不额外展示“默认”选项；3D 未声明、不可用或读取中的目录行不得显示“默认”，必须显示“未声明”或“不可用”等真实不可下发状态。切换目标应用时必须先同步更新 `selectedAppKey` 和局部 loading 并立即刷新“应用策略”区域，3D 权限详情、托管状态和账号级策略快照必须在目标应用切换链路内完成读取，不能改成 fire-and-forget 后台补齐；应用策略区域必须按目标应用 key 强制重建，避免 ArkUI 复用旧应用的 Select/状态行内部状态。页面必须在“应用策略”卡片下方提供“当前策略”表格，展示从系统策略列表、当前账号下所有应用状态和本地成功下发记录合并出的有效策略；系统回读仍是最终真相源，安装、卸载、运行和网络等系统可枚举策略不得用本地记录补回，避免恢复默认后陈旧记录残留；本地记录只用于删除映射和补充 3D 等无法全局枚举的策略。每条可管理策略必须提供删除按钮，点击删除先弹确认框，用户确认后才调用对应 MDM API 清除系统策略；删除处理中在按钮文字左侧展示 loading，不替换“删除”文字，成功后重新回读当前账号策略并清理本地记录。3D 权限行副标题必须说明目录/资源授权用途，不使用“未声明”作为副标题；未声明仅通过禁用态和状态值表达。目标应用保活和自启动能力暂不在权限管理页面开放，不读取、不展示、不下发、不在当前策略表生成条目。所有策略写入必须等待 MDM API 返回，本地策略记录只能在 MDM API 返回成功后写入；恢复默认、关闭卸载保护或删除当前策略成功后，必须清理对应策略记录并按系统回读刷新上方应用策略行；如果系统回读仍短暂返回旧状态，页面必须在成功回调内对当前 action 做一次精确本地同步，保证当前策略表和上方应用策略行立即一致。写入期间必须按动作粒度设置 `processingActionKey`，仅当前行、当前按钮或当前策略删除项展示 loading；失败时不得提交新 UI 状态，必须保留或恢复原状态并展示失败原因；成功后普通策略只更新当前选中应用的策略状态、摘要和当前策略表格，不刷新应用管控对象区；3D 写入后只回读当前应用的 3D 托管状态。成功不弹二次结果框，失败时才展示失败提示并记录底层错误码。运行策略下发禁止时必须按 `appIdentifier` 写入，但恢复默认时必须同时清理 `bundleName` 和 `appIdentifier`，兼容历史或系统残留策略；网络策略恢复默认必须清理当前 UID 下所有 DENY/REJECT 规则，不能只删除一个模板规则；卸载保护必须同时覆盖 `bundleName` 和 `appIdentifier`，写入后按系统回读列表确认状态，避免只写单一 ID 导致页面显示与系统卸载入口不一致。安装/卸载策略清理互斥列表时必须先读取现有策略并只移除已存在项，避免参数校验或不存在项清理导致整次失败。当前 SDK 中安装、卸载、运行、网络等企业管理策略读取接口没有 Promise/callback 异步重载，部分 API 直接以 `*Sync` 暴露；因此权限管理 ViewModel 由 `ApplicationRuntimeManager` 统一持有，允许在应用第一次启动且首页入场动画完整结束后随全局 runtime warmup 只预加载账号和应用清单；不得在预热阶段自动选择应用、读取全量策略快照或读取 3D 详情，也不得用固定延迟伪装异步刷新；同步接口读取前必须先提交局部 loading/置灰状态，避免点击下拉框后无反馈。`IconTextActionButton` 不承载 loading 状态，按钮左侧 loading 由业务页面自行组合。同步新增 `ohos.permission.GET_BUNDLE_INFO` 和 `ohos.permission.ENTERPRISE_MANAGE_USER_GRANT_PERMISSION` 权限及签名模板。直接安装/直接卸载 HAP 文件不作为本模块当前目标，避免把管控工具扩大为应用分发工具。
...
| 本地策略记录 | `PermissionPolicyLocalRepository` 使用 Preferences 保存成功下发记录 | 仅作删除映射和 3D 展示补充；安装、卸载、运行、网络等系统可枚举策略刷新时以系统回读为准，不从本地记录补回 |
