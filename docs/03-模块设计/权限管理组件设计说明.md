...
| 数据 | 真相源 | 本地缓存策略 |
|---|---|---|
| 已安装应用清单 | `ApplicationInventoryRepository` 调用 API26 `bundleManager.getInstalledBundleList(GET_BUNDLE_INFO_WITH_APPLICATION | GET_BUNDLE_INFO_WITH_SIGNATURE_INFO)` | 不长期缓存，页面刷新重新读取；API26 接口返回系统可见应用清单，当前选中账号只用于构造页面应用主键和后续账号级策略读写 |
...
- `PermissionRefreshCoordinator` 只持有非响应式刷新 Promise、完整刷新标记和账号事件 pending 状态，负责普通刷新复用、完整刷新排队以及账号事件尾部补刷；不得持有 `PermissionPageState`、调用 `PermissionService`、修改业务状态或使用固定延时串行化。
...
| Refresh Coordinator UT | `entry/src/test/permission-manage/refresh-coordinator.test.ets` | 普通刷新复用、完整刷新排队、账号事件尾部补刷、补刷期间再入事件、false/异常释放内部状态 |
