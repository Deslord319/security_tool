...
为解决目标应用切换卡顿、已下发策略读回仍显示默认、删除/还原后上下状态不同步的问题，本轮设计以 ViewModel 作为唯一业务状态源，`AsyncSelectRow` 不再维护乐观选中值或成功后等待父组件回填值；下拉框业务值来自父级传入的 `selectedIndex`。组件允许持有本次选择事件的本地 pending guard 和旧值镜像，用于避免重复触发、立即显示行内 loading，并在失败或异常时显式回退到操作前的旧 index；但组件不得提前显示目标新值。策略行只有在 MDM 成功并由 ViewModel 归并状态区后才更新 `selectedIndex`，失败时保持旧值。
