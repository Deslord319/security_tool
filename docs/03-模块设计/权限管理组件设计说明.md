...
- **目标 ViewModel 辅助层**: `entry/src/main/ets/viewmodels/permission-manage/PermissionRefreshCoordinator.ets`、`PermissionStateReducer.ets`、`PermissionAccountRefreshUseCase.ets`、`PermissionPolicyActionUseCase.ets`、`PermissionPolicyRecordMapper.ets`。
- **目标页面组件**: `entry/src/main/ets/components/permission-manage/PermissionTargetPanel.ets`、`PermissionApplicationPolicyPanel.ets`、`PermissionPolicyTable.ets`。
...
权限管理刷新边界分为七类：首次初始化、页面入场校准、账号变化事件刷新、应用变化事件刷新、手动完整校准、操作前上下文校验和写后精确同步。首次初始化读取账号列表、默认账号应用清单和账号级策略快照；页面重新进入时只通过 `refreshApplicationList()` 刷新当前账号应用清单和账号级策略快照，不重复读取已由 `AccountRuntimeService` 实时维护的账号列表；账号变化事件由 UI 进程 `AccountRuntimeService` 订阅账号 CommonEvent 后记录最新账号 payload，已初始化的 `PermissionViewModel` 调用 `refreshForAccountChange()` 刷新账号列表、当前有效账号应用清单和账号级策略快照；应用新增、卸载、更新事件由 `ManagedEventSubscriptionService` 统一订阅 MDM `MANAGED_EVENT_BUNDLE_ADDED`、`MANAGED_EVENT_BUNDLE_REMOVED`、API26 `MANAGED_EVENT_BUNDLE_UPDATED`，通过模块内 CommonEvent 桥接到 UI 进程 `ApplicationInventoryRuntimeService`，已初始化的 ViewModel 调用 `refreshForApplicationChange()` 只刷新系统可见应用清单并按当前选中账号重新合并策略快照，不重复读取账号列表。订阅服务必须在企业管理员扩展启动/启用及 UI runtime 绑定时幂等调用，确保应用升级后即使系统不重新触发 `onAdminEnabled`，新增事件列表也能重新注册；停用管理员时由扩展统一取消。手动刷新继续通过 `refreshAccountsAndApplicationList()` 完整校准账号列表和当前有效账号数据，作为事件遗漏、应用未运行或系统延迟时的人工兜底。页面入场刷新与账号/应用事件碰撞时不得因 `state.loading` 丢弃事件，ViewModel 必须通过非响应式单任务协调器等待当前刷新后继续执行对应范围的尾部补刷；账号事件和应用事件必须进入同一事件刷新队列，账号刷新优先且可覆盖在其开始前已经排队的应用刷新，事件刷新执行期间新到达的事件必须保留为下一轮尾部补刷；同类连续事件合并，禁止账号刷新和应用刷新并行写同一份 ViewModel 状态。任一应用清单或策略快照查询失败时，本轮刷新整体失败，保留上一次已确认的账号、应用、选中目标和策略状态，不得把异常等同于空清单，也不得推进对应事件已处理时间戳。策略写入成功后仍按本次成功动作精确更新 ViewModel 状态表、摘要和当前策略表；策略写入或当前策略删除发起前必须执行最小上下文校验，确认当前账号、目标应用或目标策略记录仍存在且仍指向同一应用实例。若账号已删除，ViewModel 先同步最新账号和有效账号应用清单、清空旧目标应用，再拒绝本次旧账号操作并提示重新选择账号。若目标应用已卸载，或同 `selectedAppKey` 的 `appId`、`appIdentifier`、`appIndex` 等身份字段已变化，ViewModel 先同步最新应用清单、清空旧目标应用，再拒绝本次旧应用操作并提示重新选择应用。若当前策略删除前刷新后已找不到目标记录，或目标记录已不再指向同一应用实例，ViewModel 同步最新状态并提示策略已变化，不再使用旧记录调用系统删除。禁止安装是账号级操作，仅校验当前账号仍存在，不读取目标应用清单。

目标应用清单优先由 MDM 应用新增、卸载和更新托管事件实时校准。企业管理员扩展只发布结构化 `source`、`bundleName`、可选 `accountId` 和 `timestamp`，不将托管事件直接写入日志，也不以事件载荷替代系统应用清单；跨进程 CommonEvent 的发布目标和订阅发布者都必须限制为 SecurityTool 自身包名，避免其他应用注入刷新事件或接收内部载荷。UI 进程收到事件后始终回读系统清单和当前账号策略快照。权限管理页面继续在“应用管控对象”区域保留手动刷新入口，作为 UI 进程未运行、事件订阅失败、事件延迟或设备不支持时的完整校准兜底。当前账号仍存在时保持当前账号，新增应用进入目标应用下拉但不自动选中；目标应用被卸载、更新后身份变化或重装时清空目标应用，应用策略区回到未选择状态，并重建当前策略表和摘要。页面入场的当前账号应用刷新不得让账号选择行显示 loading 动效，刷新期间仅禁用账号选择和相关操作；手动刷新仍只在刷新按钮区域显示局部 loading，不弹全局 loading。
...
底层 MDM 调用仍按接口要求传入 `accountId` 参数；应用清单使用 API26 `bundleManager.getInstalledBundleList(flags)` 与 `ohos.permission.ENTERPRISE_GET_INSTALLED_BUNDLE_LIST` 获取系统可见应用的全量基础信息和签名身份，该枚举结果本身不承诺按 `accountId` 隔离。`accountId` 只用于构造页面状态键并读取、合并该账号下的 MDM 策略，因此页面语义固定为“系统可见目标应用 + 当前选中账号策略”，不得描述为账号私有安装清单。该清单的 `ApplicationInfo.label` 可能为空或仍是 `$string:*` 资源引用，仓储层必须对过滤后的可管理应用调用 API26 `bundleManager.getApplicationLabel(bundleName, appIndex)` 和 `ohos.permission.GET_BUNDLE_INFO_PRIVILEGED` 获取当前语言下的真实应用名称，不允许页面自行解析资源。卸载保护和运行策略只使用系统 `SignatureInfo.appId` 作为策略 ID，不能把 `accountId:bundleName`、纯 `bundleName` 或 `appIdentifier` 当作 `appIds` 下发；缺少 `appId` 时必须下发失败并保留原状态。网络策略系统侧使用最新 `uid` 下发和清理，但 ViewModel 侧的状态归属仍按 `appKey`。`label` 仅展示，不作为页面业务主键；名称接口失败时不得展示资源引用，最终按 `bundleName` 降级。3D 应用实例中的应用身份值只使用系统真实 `SignatureInfo.appIdentifier`，为空时不回退 `bundleName` 或 `appId`。

权限管理模块建议首期只做四类应用级管控：

1. **应用清单与安装、卸载、执行控制**: 获取系统可见且非系统、非 SecurityTool 自身的已安装应用，并在当前选中账号策略作用域内管理安装白名单/黑名单、卸载保护和运行黑名单，允许白名单作为二期高风险增强。
...
- 获取系统可见的已安装应用信息，过滤掉系统应用和 SecurityTool 自身，再合并当前选中账号的策略状态。
...
| `refreshForApplicationChange()` | 应用新增、卸载、更新托管事件触发的页面态刷新入口；仅在 ViewModel 已初始化时等待在途刷新并尾部补刷系统可见应用清单及当前账号策略快照，不读取账号列表，不在未初始化时提前加载页面数据 |
...
| 数据 | 真相源 | 本地缓存策略 |
|---|---|---|
| 已安装应用清单 | `ApplicationInventoryRepository` 调用 API26 `bundleManager.getInstalledBundleList(GET_BUNDLE_INFO_WITH_APPLICATION | GET_BUNDLE_INFO_WITH_SIGNATURE_INFO)`，再对可管理应用调用 `getApplicationLabel(bundleName, appIndex)` | 不长期缓存，页面刷新重新读取；清单接口提供系统可见应用和签名身份，名称接口补齐当前语言应用名；当前选中账号只用于构造页面应用主键和后续账号级策略读写 |
...
| `ApplicationInventoryRepository` | 封装 API26 标准 `bundleManager.getInstalledBundleList(flags)`、系统应用和自应用过滤；不再使用需要管理员 Want 与账号参数的旧 MDM 应用清单接口 |
| `ApplicationInventoryEventBus` | 将企业管理员扩展收到的应用新增、卸载、更新托管事件转换为模块内结构化 CommonEvent；只承载刷新信号，不承载应用清单或策略真相 |
| `ApplicationInventoryRuntimeService` | UI 进程订阅应用清单变化 CommonEvent、保存最新合法 payload 并交由 `ApplicationRuntimeManager` 刷新已初始化的权限 ViewModel；停止时必须取消订阅 |
| `ManagedEventSubscriptionService` | 维护账号与应用托管事件的唯一订阅列表；企业管理员扩展与 UI runtime 只调用幂等注册入口，不得各自复制事件数组；管理员停用时统一取消订阅 |
...
- `PermissionRefreshCoordinator` 只持有非响应式刷新 Promise、完整刷新标记以及账号/应用事件的统一队列 pending 状态，负责普通刷新复用、完整刷新排队、账号事件优先覆盖和事件尾部补刷；不得持有 `PermissionPageState`、调用 `PermissionService`、修改业务状态或使用固定延时串行化。
...
- 页面只负责生命周期、区块组合、ViewModel 动作转发、账号切换 loading、失败弹窗和删除确认；账号/目标应用选择与禁止安装输入由 `PermissionTargetPanel` 展示，当前策略表由 `PermissionPolicyTable` 展示。两个子组件只消费 `@Param` 并通过 `@Event` 上报业务标识，不直接依赖 ViewModel、Service、Repository 或 DialogService。
...
| 组件 | 职责 |
|---|---|
| `PermissionTargetPanel` | 账号选择、禁止安装身份输入、目标应用选择和清单空态；只上报 `accountId`、`appKey` 和输入/提交事件，不持有业务状态 |
| `PermissionApplicationPolicyPanel` | 选中应用的卸载、运行、网络和 3D 策略配置；只从父级参数推导显示状态，不缓存策略副本 |
| `PermissionPolicyTable` | 当前策略 loading/空态、表头、策略行和删除按钮局部交互；只上报待删除的 `recordKey`，确认和业务调用由页面负责 |
...
| 能力 | 目标接口 | 权限 |
|---|---|---|
| 获取全部应用信息 | API26 `bundleManager.getInstalledBundleList(bundleFlags)` | `ohos.permission.ENTERPRISE_GET_INSTALLED_BUNDLE_LIST` |
| 获取应用展示名称 | API26 `bundleManager.getApplicationLabel(bundleName, appIndex)` | `ohos.permission.GET_BUNDLE_INFO_PRIVILEGED` |
...
3. 初始骨架阶段完成模块范围、账号/应用摘要空态、账号选择和管理员依赖提示；当前版本已继续完成下述 MDM 策略读写能力。
...
| Service / ViewModel UT | `entry/src/test/permission-manage/viewmodel.test.ets` | 应用清单或策略快照读取失败时本次刷新失败并保留旧状态；页面刷新与账号/应用事件串行；连续事件合并为尾部补刷，不回退主线程同步读取 |
| Refresh Coordinator UT | `entry/src/test/permission-manage/refresh-coordinator.test.ets` | 普通刷新复用、完整刷新排队、账号/应用统一事件队列、账号事件覆盖已排队应用事件、执行期间再入事件尾部补刷、false/异常释放内部状态 |
| ohosTest | `entry/src/ohosTest/ets/test/simple/RouteAction.test.ets` | 路由可达和主页面基础渲染 |
| 设备 POC | 新增专项用例 | 3D 托管、运行禁止、网络禁止 |

验收口径：

- 权限管理入口可从侧边栏和首页快捷入口进入，帮助与反馈仅保留在顶部菜单。
...
- 企业管理员扩展必须订阅应用新增、卸载和 API26 更新托管事件；UI 进程运行且权限 ViewModel 已初始化时，新增应用自动进入当前账号目标应用清单，卸载应用自动移除，更新或重装导致身份变化时自动清空旧目标应用并重建策略表。连续事件不得触发并行全量读取，也不得把托管事件直接写成第二份应用日志。
...
| 版本 | 日期 | 作者 | 变更 |
|---|---|---|---|
| 1.5.31 | 2026-08-13 | Codex | 收口应用事件刷新可靠性：应用清单读取失败保留上次已确认状态；账号/应用事件进入同一串行队列并按覆盖范围合并；内部 CommonEvent 限制同包发布与接收；明确清单为系统可见应用、账号仅作为策略作用域。 |
| 1.5.30 | 2026-08-12 | Codex | 修复目标应用未获取应用名的问题：保留 API26 `getInstalledBundleList(flags)` 枚举和已有 `ENTERPRISE_GET_INSTALLED_BUNDLE_LIST` 权限，对过滤后的可管理应用新增 API26 `getApplicationLabel(bundleName, appIndex)` 解析当前语言真实名称，并同步新增 `GET_BUNDLE_INFO_PRIVILEGED` 权限；名称接口失败时仅回退包名，不展示 `$string:*` 资源引用。 |
| 1.5.29 | 2026-08-12 | Codex | 评估 API26 新能力后仅接入高价值的应用清单变化链路：统一订阅应用新增、卸载和 `MANAGED_EVENT_BUNDLE_UPDATED`，通过模块内 CommonEvent 桥接 UI 进程并尾部合并刷新当前账号应用清单与策略快照；订阅列表由单一服务维护，并在企业扩展和 UI runtime 启动时幂等注册，解决升级 HAP 后 `onAdminEnabled` 不重放导致新事件未注册的问题。保留手动刷新作为进程未运行、订阅失败或设备不支持时的兜底，不接入会与审计重复的应用启动/停止日志，也不在缺少真机策略载荷样本时接入 `POLICIES_CHANGED`。 |
| 1.5.28 | 2026-08-12 | Codex | 应用清单读取切换到 API26 标准 `bundleManager.getInstalledBundleList(flags)` 和新增 `ohos.permission.ENTERPRISE_GET_INSTALLED_BUNDLE_LIST`，移除旧 MDM `getInstalledBundleList(admin, accountId, flags)` 及 `ENTERPRISE_GET_ALL_BUNDLE_INFO`，不保留兼容回退；选中账号继续用于页面主键和后续账号级策略，清单转换仅保留页面快照实际消费的字段。 |
| 1.5.27 | 2026-07-21 | Codex | 完成权限管理职责拆分：页面保留生命周期、组合和反馈，账号/目标应用区与当前策略表分别由无 ViewModel/Service 依赖的专属组件承接；刷新 Promise 与账号事件尾部补刷由无业务状态依赖的 `PermissionRefreshCoordinator` 承接。状态模型、MDM 调用、页面行为和验收口径不变。 |
| 1.5.26 | 2026-07-20 | Codex | 清理已完成后的旧“首版只读骨架”描述，明确当前版本已开放账号级禁止安装、卸载保护、运行、网络和 3D 权限策略读写；不改变模块行为和验收口径。 |
