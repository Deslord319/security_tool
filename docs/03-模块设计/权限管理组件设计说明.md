...
`AsyncSelectRow` 只负责本次异步选择的 pending、loading 和失败回退，不承载 `appKey`、策略类型或跨渲染业务状态，也不缓存成功后的显示值。下发成功后，ViewModel 更新状态表，页面重新读取父级 `selectedIndex`；失败时状态表不变，组件保持旧值。
...
权限管理初始化必须是一次性动作。`loadInitialState()` 只允许在未初始化时构建账号和应用清单基础状态；页面重新进入、全局 runtime 预热或 `ensurePermissionRuntimeServices()` 不能再次调用初始化覆盖已读策略状态。已初始化后的清单刷新必须走 `refreshAccountsAndApplicationList()` / `refreshApplicationList()`，并保留同一 `appKey` 的策略状态；目标应用策略读取必须通过用户选择目标应用或显式详情读取链路完成。

底层 MDM 调用仍按接口要求传入 `accountId` 参数；卸载保护和运行策略只使用系统 `SignatureInfo.appId` 作为策略 ID，不能把 `accountId:bundleName`、纯 `bundleName` 或 `appIdentifier` 当作 `appIds` 下发；缺少 `appId` 时必须下发失败并保留原状态。网络策略系统侧使用最新 `uid` 下发和清理，但 ViewModel 侧的状态归属仍按 `appKey`。`label` 仅展示，不作为页面业务主键；即使 SDK 结构字段名为 `appIdentifier`，3D 应用实例中的应用身份值也使用 `bundleName`。
...
| `PermissionAppSnapshot` | `appKey`、`accountId`、`bundleName`、`appId`、`label`、`uid`、`appIndex`、`uninstallProtected`、`runningBlocked`、`networkBlocked`、`threeDGroups`、`threeDPolicies`、`threeDLoadState`、`threeDMessage` | ViewModel 应用策略状态表行；应用策略区域和当前策略表均从该模型派生 |
...
| `PermissionLocalPolicyRecord` | `recordId`、`accountId`、`appKey`、`bundleName`、`appIndex`、`policyType`、`policyValue`、`status`、`createdAt`、`updatedAt`、`lastError` | 本地成功下发记录；仅保存 3D 非默认状态，用于展示补充和删除映射 |
...
flowchart TD
  ...
  E --> F["构建 appSnapshots 状态表"]
  F --> G["派生 filteredAppKeys、policyRecords、summary"]
  G --> H["选择应用"]
  H --> I["提交选中态和局部 loading"]
  I --> J["读取账号级策略快照"]
  I --> K["读取当前应用 3D 详情"]
  J --> L["归并 appSnapshots 和 installPolicies"]
  K --> L
  L --> M["统一提交状态并重建派生表"]
...
| 验收 | 输入完整包名可直接写入禁止安装；输入应用名、半截包名或非法包名时拒绝下发并提示；详情区可设置并回读运行允许（`default`/`allowed` 均显示为允许）、禁止和卸载保护状态；运行和卸载保护策略均只使用 `SignatureInfo.appId` 下发、回读和恢复默认；缺少 `appId` 时下发失败并保留原状态；失败保留原状态并说明原因 |
