...
- **目标 Service / Repository**: `entry/src/main/ets/services/permission-manage/PermissionService.ets`、`ApplicationInventoryRepository.ets`、`ApplicationControlRepository.ets`、`ApplicationNetworkPolicyRepository.ets`、`SixDPermissionRepository.ets`、`PermissionAccountProvider.ets`。
...
- **当前约束**: 本轮开放应用管控设置能力：禁止安装走账号维度输入框，只允许用户输入完整应用包名并写入禁止安装列表，不再做应用名或 `appIdentifier` 模糊映射，也不再在当前已安装应用策略行里切换安装策略；当前账号、目标应用、卸载保护、运行策略、网络策略和 3D 权限托管均使用外设接口管控同款 `AsyncSelectRow`，下拉过程中左侧显示 loading、控件置灰并在失败时回滚；当前账号切换成功后按新账号刷新应用清单、清空目标应用和当前策略表，失败时保持旧账号。目标应用默认不自动选择，下拉框第一项固定为“请选择应用”；用户选择具体应用后控件显示目标应用名称，不再显示“请选择应用”占位文本，下拉框宽度与禁止安装输入框一致；每次进入权限管理页面都必须先重新读取账号列表，保留仍然存在的当前账号；若当前账号已不存在，则回退到账号 Provider 返回的前台账号或首个账号，再按有效当前账号重新读取一次应用清单，确保新增用户、新安装、卸载或重装的应用能进入对应下拉列表，但不联动读取当前选中应用的 3D 详情或账号级策略快照；目标应用列表必须过滤 `systemApp === true` 的系统应用和 SecurityTool 自身，FormRenderService、ArkWeb、长时任务通知资源等系统框架或资源包也必须强制隐藏；不得再用 `removable === false` 隐藏目标应用，避免企业包或调试包被误伤；目标应用以包名和应用 key 做唯一映射，label 只作为展示字段，不允许用 label 查找、写入或删除策略；下拉选项必须带包名以区分同名应用。用户选择具体应用后先同步提交选中态并显示局部读取转圈，目标应用行按外设管理下拉框样式在下拉框左侧显示局部读取转圈，读取期间下拉框置灰；切换目标应用必须在本次选择动作内读取当前应用 3D 权限详情和账号级策略快照，确保应用策略行与当前策略表同步更新；读取前先提交局部 loading/置灰状态，避免同步 MDM 接口执行期间无反馈。“当前策略”表格按当前账号展示所有应用和安装策略，不按目标应用过滤。权限管理应用清单只请求应用基础信息和签名信息，不请求应用图标。卸载保护默认“未启用”且不再弹二次确认；运行策略和网络策略下拉只展示“允许/禁止”，其中“允许”对应系统默认放行/恢复默认，不额外展示“默认”选项；3D 未声明、不可用或读取中的目录行不得显示“默认”，必须显示“未声明”或“不可用”等真实不可下发状态。切换目标应用时必须先同步更新 `selectedAppKey` 和局部 loading 并立即刷新“应用策略”区域，3D 权限详情、托管状态和账号级策略快照必须在目标应用切换链路内完成读取，不能改成 fire-and-forget 后台补齐；应用策略区域必须按目标应用 key 强制重建，避免 ArkUI 复用旧应用的 Select/状态行内部状态。页面必须在“应用策略”卡片下方提供“当前策略”表格，展示从系统策略列表、当前账号下所有应用状态和本地 3D 托管记录合并出的有效策略；系统枚举仍用于目标应用选择和后续校准，安装、卸载、运行和网络等系统可枚举策略不得用本地记录补回，避免恢复默认后陈旧记录残留；本地记录只用于删除映射和补充 3D 等无法全局枚举的策略。每条可管理策略必须提供删除按钮，点击删除先弹确认框，用户确认后才调用对应 MDM API 清除系统策略；删除处理中在按钮文字左侧展示 loading，不替换“删除”文字，成功后根据对应策略记录更新 ViewModel 状态区并清理必要本地记录。3D 权限行副标题必须说明目录/资源授权用途，不使用“未声明”作为副标题；未声明仅通过禁用态和状态值表达。目标应用保活和自启动能力暂不在权限管理页面开放，不读取、不展示、不下发、不在当前策略表生成条目。所有策略写入必须等待 MDM API 返回；恢复默认、关闭卸载保护或删除当前策略成功后，必须根据本次成功动作更新 ViewModel 状态区，并同步重建上方应用策略行、摘要和当前策略表；失败时不得提交新状态。写入期间必须按动作粒度设置 `processingActionKey`，仅当前行、当前按钮或当前策略删除项展示 loading；失败时不得提交新 UI 状态，必须保留或恢复原状态并展示失败原因；成功后普通策略只按本次成功动作更新当前选中应用的策略状态、摘要和当前策略表格，不刷新应用管控对象区；3D 写入后只回读当前应用的 3D 托管状态。成功不弹二次结果框，失败时才展示失败提示并记录底层错误码。运行策略、卸载保护和应用级策略映射均必须只按 `bundleName` 写入、读取和展示，不使用签名 `appIdentifier` 作为策略 ID，避免同签名或调试应用被一条策略同时命中；网络策略恢复默认必须清理当前 UID 下所有 DENY/REJECT 规则，不能只删除一个模板规则。安装/卸载策略清理互斥列表时必须先读取现有策略并只移除已存在项，避免参数校验或不存在项清理导致整次失败。当前 SDK 中安装、卸载、运行、网络等企业管理策略读取接口没有 Promise/callback 异步重载，部分 API 直接以 `*Sync` 暴露；因此权限管理 ViewModel 由 `ApplicationRuntimeManager` 统一持有，允许在应用第一次启动且首页入场动画完整结束后随全局 runtime warmup 只预加载账号和应用清单；不得在预热阶段自动选择应用、读取全量策略快照或读取 3D 详情，也不得用固定延迟伪装异步刷新；同步接口读取前必须先提交局部 loading/置灰状态，避免点击下拉框后无反馈。`IconTextActionButton` 不承载 loading 状态，按钮左侧 loading 由业务页面自行组合。同步新增 `ohos.permission.GET_BUNDLE_INFO` 和 `ohos.permission.ENTERPRISE_MANAGE_USER_GRANT_PERMISSION` 权限及签名模板。直接安装/直接卸载 HAP 文件不作为本模块当前目标，避免把管控工具扩大为应用分发工具。

### 设计结论

#### 本轮状态源收敛

为解决目标应用切换卡顿、已下发策略读回仍显示默认、删除/还原后上下状态不同步的问题，本轮设计以 ViewModel 作为唯一业务状态源，应用级策略状态只存放在 `PermissionPageState.applications` 中，页面应用策略行通过 `selectedAppKey` 从 `applications` 派生当前应用；不得再维护独立的 `selectedApplication` 对象副本，避免上方应用策略和下方当前策略表读取两份对象。`AsyncSelectRow` 不做提前乐观变更；组件允许持有本次选择事件的本地 pending guard 和旧值镜像，用于避免重复触发、立即显示行内 loading，并在失败或异常时显式回退到操作前的旧 index；组件只能在 `onSelectAsync` 返回成功后切到本次选择值。策略行只有在 MDM 成功并由 ViewModel 归并状态区后才更新 `selectedIndex`，失败时保持旧值。

应用策略下拉成功返回后，`AsyncSelectRow` 只允许保留本次异步选择的 pending 和失败回退状态；动作成功后更新内部显示镜像，后续非 pending 渲染必须以父级 `selectedIndex` 为准。组件不得持有“等待父状态追上”的跨渲染业务状态，也不得由父页面额外传入业务身份状态；切换目标应用时，上方卸载保护、运行、网络和 3D 下拉应自然跟随 ViewModel 派生出的当前应用 `selectedIndex`，避免复用上一应用成功值。

目标应用选择必须先提交 `selectedAppKey`、`selectedAppRefreshing` 和 `policyLoading`，让目标应用行与应用策略区域进入局部 loading/置灰状态，再执行系统读取。账号级系统策略读取必须由 Service 严格读取原始系统策略，读取失败不得返回空列表兜底，也不得把失败解释为系统默认策略；只有读取成功后才允许将安装、卸载、运行、网络策略归并到 `applications`，再由 `applications` 派生当前策略表和摘要。若账号策略快照读取失败，本次目标应用选择必须返回失败并回退到选择前状态。`TimedLoadingDialogRunner` 只负责 loading 可见性，不等同于后台线程。选择链路仍在同一次动作内完成账号级策略快照和当前应用 3D 详情读取，并通过请求序号丢弃旧选择返回的数据。

当前 SDK 中部分 MDM 读写接口为同步接口或会在 UI 线程产生明显等待，页面层必须使用 `TimedLoadingDialogRunner` 包裹目标应用切换、当前账号切换、策略下发、禁止安装和当前策略删除等耗时动作；动作开始后仍按行级 `processingActionKey` 或 `selectedAppRefreshing` 展示局部 loading，并置灰对应下拉框或按钮。同步读取类动作必须返回明确成功或失败，不能使用空数组、默认值或本地记录作为系统读取失败的替代结果。

安装、卸载、运行和网络策略属于系统可枚举策略，页面状态和“当前策略”表格必须以系统快照为最终真相源，但单次 MDM 写入返回成功后，ViewModel 必须按本次成功动作精确同步当前应用的 `uninstallProtected`、`runningPolicy` 或 `networkPolicy` 字段，并立即重建摘要和当前策略表，避免刚写入的单项被延迟或不完整的全量枚举读回覆盖成旧值。该即时同步仅使用当前应用 `app.key = accountId + bundleName` 作为业务身份，不使用 label、`appIdentifier`、单独 bundleName 或旧版本复合 key 映射，也不写入普通策略本地记录；底层 MDM 调用仍按接口要求传入 `accountId` 参数和 `bundleName` 策略 ID，网络规则仅在系统 API 层使用当前应用 `uid` 下发和清理。失败时不提交新状态。恢复默认、关闭卸载保护或从当前策略表删除成功后，同样按成功动作精确删除对应当前策略记录并同步上方策略行。后续页面进入、账号切换或目标应用切换仍可重新读取系统快照校准状态。

本地策略记录仅保留两类职责：一是 3D 权限托管这类系统无法按账号全量枚举的策略补充展示；二是在删除 3D 记录时映射回目标应用和权限组。禁止安装、卸载保护、运行策略和网络策略不再写入本地策略记录，也不得从本地记录补回当前策略表。合并本地 3D 记录时必须校验 `record.appKey` 存在于当前账号的应用清单，旧版本主键、已卸载应用或其他账号残留记录不得进入当前策略表；3D 记录删除成功后，如果记录属于当前选中应用，必须同步把 `selectedPermissionProfile` 中对应权限组恢复为默认态，让上方 3D 下拉和下方当前策略表从同一份 ViewModel 状态刷新。

应用清单中的 `appIdentifier` 仅作为系统原始信息保留，不参与权限管理业务主键、策略匹配、策略写入、删除或去重。包名是同账号下目标应用选择和应用级策略映射的唯一身份，label 只用于展示；即使 SDK 结构字段名为 `appIdentifier`，权限管理传入的应用身份值也使用 `bundleName`。
...
| 接口 | 说明 |
|---|---|
| `loadInitialState()` | 初始化管理员状态、账号列表、默认账号应用清单和基础摘要；不自动选择目标应用，不读取策略快照和 3D 详情 |
| `switchAccount(accountId)` | 切换账号并重新加载应用清单，清空当前选中应用和当前策略表；策略快照由后续目标应用选择、写操作或删除操作触发 |
| `refreshApplications()` | 按当前账号刷新非系统应用清单；账号切换等主动刷新使用，不读取当前选中应用 3D 详情和账号策略快照 |
| `refreshApplicationList()` | 页面每次重新进入时调用，只刷新目标应用清单并保留当前策略表，不联动强制刷新选中应用详情 |
...
| `selectApplicationAndLoadDetails(appKey)` | 先调用 `selectApplication` 并提交局部 loading，再在本次选择动作内读取当前应用 3D 权限详情和账号级策略快照；账号级策略读取成功后统一归并到 `applications`，旧应用详情或快照返回时必须丢弃 |
| `setDisallowedInstallPolicyId(accountId, policyId)` | 按账号写入禁止安装策略；`policyId` 来自用户输入的完整应用包名 |
| `setUninstallProtected(appKey, enabled)` | 设置或取消卸载保护 |
| `setRunningPolicy(appKey, policy)` | 设置运行允许（恢复默认放行）或禁止策略 |
| `setSixDPermissionState(appKey, groupKey, state)` | 设置 3D 权限托管状态，目标应用未声明权限时不可下发并返回明确失败 |
...
- 3D 权限托管使用 `ApplicationInstance`，应用身份值按 `bundleName` 传入，并携带 `accountId`、`appIndex`。
...
| `ManagedApplicationInfo` | `key`、`accountId`、`bundleName`、`appIdentifier`、`appIndex`、`uid`、`label`、`versionName`、`systemApp`、`removable`、`installSource`、`appDistributionType`、`iconData` | 应用清单主模型；`iconData` 为空时页面不显示应用图标，也不使用通用占位图 |
| `ApplicationPermissionProfile` | `requestedPermissions`、`sixDGroups`、`sensitivePermissionCount` | 目标应用声明权限和 3D 分组结果 |
| `SixDPermissionItem` | `groupKey`、`title`、`permissionNames`、`declaredPermissions`、`managedState`、`canManage`、`disabledReason` | 3D 权限托管行 |
| `ApplicationControlState` | `installPolicy`、`uninstallProtected`、`runningPolicy`、`networkPolicy`、`conflicts` | 应用控制聚合状态 |
| `PermissionPolicyRecord` | `key`、`policyType`、`targetName`、`targetDetail`、`statusLabel`、`scopeLabel`、`actionKind`、`appKey`、`policyId` | 当前策略表格展示模型；由系统策略列表和应用状态派生，不作为历史记录持久化；删除时按 `actionKind` 映射到对应清理 API |
| `PermissionSummary` | `appCount`、`managedCount`、`sixDSensitiveCount`、`networkRestrictedCount` | 页面摘要卡数据；策略回读完成后 `managedCount` 代表当前策略表格条目数 |
| `PermissionLocalPolicyRecord` | `recordId`、`accountId`、`appKey`、`bundleName`、`appIdentifier`、`appIndex`、`policyType`、`policyValue`、`status`、`createdAt`、`updatedAt`、`lastError` | 本地成功下发记录；用于展示和删除映射，不替代系统策略真相源 |
| `PermissionPageState` | `adminReady`、`accounts`、`selectedAccountId`、`apps`、`filteredApps`、`selectedAppKey`、`searchKeyword`、`filters`、`summary`、`policyRecords`、`loading`、`policyLoading`、`processingAction`、`processingActionKey`、`error`、`inventoryMessage`、`selectedPermissionProfile` | ViewModel 页面状态；应用策略区域通过 `selectedAppKey` 从 `applications` 派生当前应用，`processingActionKey` 用于行级 loading 和防重复提交 |
...
flowchart TD
  ...
  E --> G["展示应用清单和摘要"]
  G --> H["选择应用"]
  H --> I["提交选中态和局部 loading"]
  I --> J["读取声明权限和 3D 托管状态"]
  J --> N["读取账号级策略快照"]
  N --> K["执行权限托管或恢复默认"]
  N --> L["执行安装、卸载、运行和网络策略变更"]
  K --> M["按成功动作更新状态区并重建策略表"]
  L --> M
```
...
| 3D 托管状态 | `SixDPermissionRepository` 调用 `enterprise.securityManager.getPermissionManagedState` / `setPermissionManagedState` | 不作为真相源持久化 |
...
| 本地策略记录 | `PermissionPolicyLocalRepository` 使用 Preferences 保存 3D 托管记录 | 仅作 3D 删除映射和展示补充；安装、卸载、运行、网络等系统可枚举策略不写入本地记录 |
...
| 安装控制 | 在账号作用域输入完整应用包名，校验通过后写入禁止安装列表；当前应用策略区不提供安装策略下拉 |
...
| 验收 | 输入完整包名可直接写入禁止安装；输入应用名、半截包名或非法包名时拒绝下发并提示；详情区可设置并回读运行允许（`default`/`allowed` 均显示为允许）、禁止和卸载保护状态；运行和卸载保护策略均使用 `bundleName` 下发与回读；失败保留原状态并说明原因 |
...
- 写操作必须等待 MDM API 返回；普通应用策略、禁止安装和当前策略删除成功后由 ViewModel 更新状态区并重建策略表，不再做账号级强制刷新；3D 成功后只回读当前应用权限详情。
...
- 管控对象区只展示选择控件，不再额外展示包名、版本、UID 三行详情。
- 目标应用默认不自动选中，未选时“应用策略”展示“请选择应用”；一旦选中具体应用，目标应用控件显示目标应用名称，不再显示“请选择应用”占位文本。选中应用后，下面用类似身份鉴别页面的 `SectionCard` 一条一条展示策略项；目标应用、卸载保护、运行策略、网络策略和 3D 托管态均使用外设接口管控同款 `AsyncSelectRow`，卸载保护默认“未启用”且选择后直接下发，不再弹确认框；运行/网络策略只展示“允许/禁止”，允许对应恢复默认放行；3D 未声明/不可用/读取中行不得显示为“默认”。
...
- 首次进入页面时，账号和应用清单加载完成后必须先展示主体内容；之后每次重新进入权限管理页面必须刷新账号列表并保留仍存在的当前账号，当前账号不存在时回退到账号 Provider 返回的前台账号或首个账号，再刷新有效当前账号应用清单，避免新增用户、安装、卸载或重装后账号/目标应用下拉仍使用旧缓存，但不联动强制回读已选应用的 3D 详情或账号级策略快照；如果目标应用已被卸载或过滤掉，则清空目标应用和详情。默认目标应用为空，不自动读取策略列表和 3D 权限详情，不得让全页摘要和管控对象区长期显示“正在加载”。用户选择目标应用后，先同步显示选中态和局部读取转圈，再在本次选择动作内读取当前应用 3D 权限详情和账号级策略快照；同步读取前由下拉组件让 loading/置灰状态先渲染，不能改成 fire-and-forget 后台补齐。
...
| `EmptyStatePanel` | 无可管控非系统应用、无 3D 权限、无网络策略 |
| `SettingsSectionCard` | 应用详情里的控制分组 |
| `SectionSelectRow` | 当前账号等无需失败回滚的普通选择行 |
| `AsyncSelectRow` | 目标应用、卸载保护、运行策略、网络策略、3D 托管态选择；写入中左侧 loading、置灰并失败回滚 |
...
| `SixDPermissionPanel` | 3D 权限组展示和托管态选择 |
...
首期不新增 RDB 表。系统策略以系统接口回读为准，本地策略记录使用 Preferences JSON 列表保存。
...
- 最近选中的策略分组折叠状态。
- UI 折叠状态。
- 成功下发的权限管理策略记录，状态为 `active` 或 `stale`；系统回读冲突时以系统为准。
...
- 同步接口写入期间，当前行或按钮必须显示 loading；失败后控件回到旧值，当前策略表不增加未确认记录。
