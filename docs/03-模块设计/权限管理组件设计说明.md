...
| `selectApplicationAndLoadDetails(appKey)` | 先调用 `selectApplication` 并提交局部 loading，页面侧不等待详情读取完成即可关闭本次下拉选择；随后后台读取当前应用 3D 权限详情和账号级策略快照，成功后统一归并到 `applications`，旧应用详情或快照返回时必须丢弃 |
...
| `PermissionLocalPolicyRecord` | `recordId`、`accountId`、`appKey`、`bundleName`、`appIdentifier`、`appIndex`、`policyType`、`policyValue`、`status`、`createdAt`、`updatedAt`、`lastError` | 本地成功下发记录；仅保存 3D 非默认状态，用于展示补充和删除映射 |
...
策略快照读取链路允许在 Service 层通过 `taskpool` 执行同步 MDM 查询，避免 `get*Sync` 和防火墙规则枚举阻塞 ArkUI 主线程。后台任务只返回账号级原始策略列表和受限 UID 列表，不直接持有或修改页面状态；ViewModel 仍在主线程完成 `appSnapshots`、`installPolicies`、`policyRecords` 和 `summary` 的统一提交与旧请求丢弃。若 `taskpool` 执行失败，Service 必须回退到原同步读取并保留失败日志，不能让页面进入无策略假成功态。
...
| Service UT | `entry/src/test/permission-manage/viewmodel.test.ets` | 策略快照读取优先走 taskpool，失败时回退同步读取，且合并结果保持不变 |
