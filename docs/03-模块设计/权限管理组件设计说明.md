...
- **当前约束**: 本轮开放应用管控设置能力：禁止安装走账号维度输入框，只允许用户输入完整应用包名并写入禁止安装列表，不再做应用名或 `appIdentifier` 模糊映射，也不再在当前已安装应用策略行里切换安装策略；卸载保护、运行策略、网络禁止/恢复和 6D 权限托管均由控件选择后直接下发。页面必须在“应用策略”卡片下方提供“当前策略”表格，展示从系统策略列表和当前应用状态回读出的有效策略，不保存本地下发历史；每条可管理策略必须提供删除按钮，点击删除先弹确认框，用户确认后才调用对应 MDM API 清除系统策略，成功后重新回读当前账号策略。目标应用保活和自启动能力暂不在权限管理页面开放，不读取、不展示、不下发、不在当前策略表生成条目。所有策略写入必须等待 MDM API 返回，成功后普通策略只更新当前选中应用的策略状态、摘要和当前策略表格，不刷新应用管控对象区；6D 写入后只回读当前应用的 6D 托管状态。成功不弹二次结果框，失败时才展示失败提示并记录底层错误码。运行策略必须按 `appIdentifier` 下发；卸载保护必须同时覆盖 `bundleName` 和 `appIdentifier`，写入后按系统回读列表确认状态，避免只写单一 ID 导致页面显示与系统卸载入口不一致。安装/卸载策略清理互斥列表时必须先读取现有策略并只移除已存在项，避免参数校验或不存在项清理导致整次失败。当前 SDK 中安装、卸载、运行、网络等企业管理策略读取接口没有 Promise/callback 异步重载，部分 API 直接以 `*Sync` 暴露；因此权限管理 ViewModel 必须由 `ApplicationRuntimeManager` 持有并在应用启动后预热，进入权限管理时不得在点击事件链路里触发完整策略快照，也不得用固定延迟伪装异步刷新；策略快照只在启动预热、账号/应用数据刷新和策略写入成功后触发，且同一时刻只允许一个快照任务运行。同步新增 `ohos.permission.GET_BUNDLE_INFO` 和 `ohos.permission.ENTERPRISE_MANAGE_USER_GRANT_PERMISSION` 权限及签名模板。直接安装/直接卸载 HAP 文件不作为本模块当前目标，避免把管控工具扩大为应用分发工具。
...
| `PermissionPageState` | `adminReady`、`accounts`、`selectedAccountId`、`apps`、`filteredApps`、`selectedAppKey`、`searchKeyword`、`filters`、`summary`、`policyRecords`、`loading`、`policyLoading`、`processingAction`、`error`、`inventoryMessage`、`selectedPermissionProfile` | ViewModel 页面状态 |
...
| `PermissionLocalRepository` | 可选，只保存最近选中账号和应用，不保存系统策略真相 |
...
- 选中应用后，下面用类似身份鉴别页面的 `SectionCard` + `SectionSelectRow` / `SectionToggleRow` 一条一条展示策略项。
...
- 首次进入页面时，账号和应用清单加载完成后必须先展示主体内容；策略列表和 6D 权限详情允许后台补齐，不得让全页摘要和管控对象区长期显示“正在加载”。MainPage 持有的共享权限 ViewModel 应在应用启动后预热，和日志、外设等模块保持一致；路由切换不得触发完整策略快照，避免同步 MDM 查询阻塞点击反馈。
...
- 首次点击权限管理时不应出现长时间空白、多块 Loading 或明显点击卡顿；应用启动后的后台预加载应使首次进入大多数情况下直接看到账号和应用数据，策略表格可使用已预热缓存或在写操作后精确回读。
