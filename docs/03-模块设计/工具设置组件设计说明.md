...
- **关键流转路径**:
  - `页面进入` -> `ToolSettingsViewModel.initialize(context)` -> `PreferencesToolSettingsRepository.loadConfig(context)` -> `config/initialConfig 同步` -> `initialized = true`。
...
* **结构说明**:
  * `ToolSettingsPage`: 工具设置页面入口。页面进入时初始化 ViewModel，初始化完成后展示“启动认证设置”卡片；页面头部保存按钮根据 `initialized`、`hasChanges`、`saving` 控制可用状态。
