...
- **对外暴露能力 (Public APIs)**:
  - `PreferencesToolSettingsRepository.loadConfig(context)`: 供页面初始化读取工具设置配置；读取异常时仍返回默认配置，避免设置页不可进入。
  - `PreferencesToolSettingsRepository.loadConfigStrict(context)`: 供 `EntryAbility` 启动阶段读取工具设置配置；读取异常必须向上抛出，由启动认证链路按 PIN 认证兜底处理，不允许直接放行主页面。
...
- `已初始化后从其它路由返回工具设置页面` -> 复用共享 `ToolSettingsViewModel.state` -> 保留上次退出页面时的当前编辑态与 `hasChanges / saving`，不重复读取 Preferences 覆盖未保存变更。
  ...
  - `应用启动` -> `EntryAbility.checkStartupAuth(windowStage)` -> `loadConfigStrict(context)` -> 根据 `startupAuthEnabled` 和 `authMethod` 决定是否调用 `AuthService.authenticate(authMethod)`；若配置读取异常，则按 `AuthMethod.PIN` 执行启动认证兜底，认证通过才加载主页面。
...
* **核心实体与依赖路径**:
  * `ToolSettingsConfig`
    * **所在文件**: `entry/src/main/ets/models/ToolSettingsModels.ets`
  ...
  * `AuthMethod`
    ...
    * **选择顺序**: `ToolSettingsModels.ets` 中的 `SUPPORTED_TOOL_AUTH_METHODS` 定义工具设置可选认证方式的业务顺序，ViewModel 使用该数组计算选中索引；展示层只基于该数组派生 label。
...
* **核心业务用例 (Use Cases)**:
  * `PreferencesToolSettingsRepository.loadConfig(context)`: 读取工具设置配置。
    ...
    * **失败策略**: 记录错误日志并返回 `DEFAULT_TOOL_SETTINGS`，仅供设置页初始化使用。
  * `PreferencesToolSettingsRepository.loadConfigStrict(context)`: 严格读取工具设置配置。
    * **副作用**: 无写入；读取 Preferences store `tool_settings`。
    * **失败策略**: 抛出标准 `Error`，供启动阶段识别真实读取失败并触发 PIN 认证兜底。
...
* **结构说明**:
  ...
  * `SectionSelectRow`: 展示并回传认证方式选择变化；下拉控件直接将行组件的 `currentTheme` 传给 `StyledSelect`，不通过普通参数对象二次转发，确保主题切换时选择框颜色与页面状态同帧更新。
...
* **涉及修改**:
  - `entry/src/main/ets/models/ToolSettingsModels.ets`
...
* **说明**:
  ...
  - 配置读取异常时不直接加载主页面，改为使用 `AuthMethod.PIN` 执行启动认证；PIN 可用且认证成功才加载主页面。
  - 认证方式不可用、认证结果为 `unavailable` 或认证失败时均调用 `terminateSelf`，不再降级加载主页面。
...
sequenceDiagram
  ...
  Entry->>Repo: loadConfigStrict(context)
...
- **异常兜底策略**:
  - **设置页配置读取失败**: `PreferencesToolSettingsRepository.loadConfig` 返回 `DEFAULT_TOOL_SETTINGS`。
  - **启动阶段配置读取失败**: `EntryAbility` 通过 `loadConfigStrict` 感知异常后使用 PIN 认证兜底；PIN 不可用或认证未通过时终止应用，不加载主页面。
  - **配置保存失败**: `saveConfig` 返回 `false`，ViewModel 返回保存失败结果。
  - **无变更保存**: ViewModel 返回失败结果，页面提示当前没有需要保存的更改。
  - **认证方式不可用**: 保存阶段返回确认请求；启动阶段终止应用，不加载主页面。
  - **认证失败**: 启动阶段认证失败或认证能力返回 `unavailable` 时终止应用。
...
- **验收口径**:
  - 设置页配置读取失败回退默认值，保存失败保持脏状态并允许重试；无变更保存给出明确提示。
  - 启动阶段配置读取失败时执行 PIN 认证兜底；PIN 可用且认证成功才进入主页面。
  - 不可用认证方式在保存阶段需要二次确认，启动阶段不再降级进入主页面。
...
|---|---|---|---|
| 1.0.5 | 2026-07-09 | Codex | 收紧启动认证异常策略：新增严格配置读取入口，启动阶段配置读取失败改为 PIN 认证兜底，认证不可用或失败时终止应用，不再直接放行主页面。 |
| 1.0.4 | 2026-07-07 | Codex | 修复认证方式下拉框深色/浅色模式切换时样式慢一拍：`SectionSelectRow` 直接向 `StyledSelect` 传递当前主题，不再经自由 builder 参数对象转发。 |
| 1.0.3 | 2026-05-11 | Codex | 归档工具设置模型与展示职责拆分设计：配置模型和 Preferences Key 迁入 `ToolSettingsModels.ets`，认证方式枚举迁入 `AuthModels.ets`，启动认证展示 option 迁入工具设置展示层，认证方式业务顺序由领域模型提供。 |
| 1.0.2 | 2026-05-11 | Codex | 收敛 ohosTest 验收口径到当前 dispatcher 场景，移除未接入历史页面测试引用。 |
| 1.0.1 | 2026-05-09 | Codex | 补充工具设置模块实施步骤、测试覆盖和验收口径，明确启动认证、配置持久化和系统设置入口的落地闭环；移除简化流程摘要，RFC 层级对照回归总体 RFC。 |
