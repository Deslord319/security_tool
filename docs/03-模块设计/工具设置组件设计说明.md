...
- **核心伪码**:
  ```text
  saveToolSettings(draft):
    if draft has no changes:
      return failure(NO_CHANGES)
    if startupAuthEnabled and selected auth method unavailable:
      return confirm(UNAVAILABLE_AUTH_METHOD)
    saved = ToolSettingsRepository.saveConfig(draft)
    return saved ? success : failure(SAVE_FAILED)

  EntryAbility.checkStartupAuth(windowStage):
    config = ToolSettingsRepository.loadConfig(context)
    if startupAuthEnabled is false:
      load MainPage
    else if AuthService.checkAuthMethodAvailability(config.authMethod) fails:
      load MainPage with degraded prompt
...
- **测试覆盖**:
  ...
  - ohosTest: `entry/src/ohosTest/ets/test/tool-settings/page.test.ets` 覆盖页面渲染和交互基线。
...
|---|---|---|---|
| 1.0.2 | 2026-05-09 | Codex | 补充工具设置核心伪码，明确配置保存、认证能力确认和启动认证消费流程。 |
