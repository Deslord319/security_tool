...
- **一致性原则**: UKey 能力不再放在 SecurityTool 主应用内。SecurityTool 只保留身份鉴别口令策略和通用 PIN/指纹认证能力，不再声明 CustomAuthenticator appService，不再读取 UKey，不再保存 UKey 绑定或活动凭据。
...
- **核心业务状态 (Core Business State)**:
  ...
  - `activeCredential: LockScreenUKeyActiveCredential | null`: 当前系统侧已注入的活动 CustomAuth 凭据。UKey 拔出时删除该凭据，但保留首把绑定。
...
- **关键流转路径**:
  ...
  - USB detach -> `UKeyRuntimeManager` -> 读取开关 -> `LockScreenCustomAuthEnrollmentService.onUKeyDetached()` -> 首把不在场且存在活动凭据时删除系统凭据。
  - 首次发现唯一 UKey -> 生成 fingerprint -> 调用 `addCredential` -> 保存 `trustedBinding` 与 `activeCredential`。
  - 已有首把绑定且同一把在场 -> 若活动凭据缺失则补注入；若已存在活动凭据则不重复注册。
  - 已有首把绑定后发现第二把 -> 不调用 `addCredential`，不覆盖首把绑定。
  - 首把拔出 -> 若存在对应活动凭据，调用 `delCred` 删除系统凭据，成功后清空 `activeCredential`，保留 `trustedBinding`。
...
- **拔出删除活动凭据**: 首把不在场时删除当前活动凭据，避免 UKey 不在场但系统仍保留可认证凭据。
...
- `ukey/entry/src/main/ets/models/identity/lockscreen-auth/LockScreenAuthModels.ets`
  - 保存 UKey 开关、设备、首把绑定、活动凭据和 `pluginInfo` 模型。
...
- `ukey/entry/src/main/ets/services/identity/lockscreen-auth/LockScreenUKeyBindingRepository.ets`
  - 保存首把绑定、活动凭据和模板绑定，不依赖 SecurityTool 数据。
...
- **异常兜底策略**:
  ...
  - PIN token 获取失败或 `addCredential` 返回非 0 时不保存活动凭据。
...
- **测试覆盖**:
  ...
  - 设备手工: 安装 `ukey/` 系统签名 HAP 后，第一把 UKey 可注册，第二把不注册，拔出首把删除活动凭据。
- **验收口径**:
